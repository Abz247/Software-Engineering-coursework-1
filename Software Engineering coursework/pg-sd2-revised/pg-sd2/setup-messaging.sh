#!/bin/bash
# ============================================================
# ClosetSwap Sprint 4 – Messaging Backend Setup
# Run this from: Software Engineering coursework/pg-sd2-revised/pg-sd2/
# ============================================================

echo "Setting up Messaging System..."

# ---- SQL migration file ----
cat > messaging-schema.sql << 'EOF'
-- ============================================================
...
EOF
echo "Created messaging-schema.sql"

# ---- Messages route ----
cat > app/routes/messages.js << 'EOF'
/**
...
EOF
echo "Created app/routes/messages.js"

# ---- Patch app.js to mount the messages routes ----
# Add the route require and mount if not already present
if ! grep -q "require(\"./routes/messages\")" app/app.js; then
    # Insert messages route require after userRoutes require
    sed -i.bak 's|const userRoutes = require("./routes/users");|const userRoutes = require("./routes/users");\nconst messageRoutes = require("./routes/messages");|' app/app.js

    # Insert messages mount after /users mount
    sed -i.bak 's|app.use("/users", userRoutes);|app.use("/users", userRoutes);\napp.use("/messages", messageRoutes);|' app/app.js

    rm -f app/app.js.bak
    echo "Patched app/app.js to mount /messages routes"
else
    echo "app.js already has messages routes – skipped patching"
fi

echo ""
echo "============================================"
echo "  Messaging backend setup complete!"
echo "============================================"
echo ""
echo "Next steps:"
echo "  1. Open phpMyAdmin at http://localhost:8081 (or 8082)"
echo "  2. Select your database, go to SQL tab"
echo "  3. Paste contents of messaging-schema.sql and click Go"
echo "  4. Restart Docker or wait for supervisor to restart the app"
echo "  5. Test the API endpoints (see API_DOCS.md)"
echo ""
